"""
Kaggle kernel entry point for Day 5 REAL training of AttFuse on the full
...
image (see the note in scripts/kaggle_train_entry.py's sibling,
kaggle_speed_test_kernel.py, for why cumm/spconv/numba aren't among
them), then hands off to scripts/kaggle_train_entry.py --model attfuse,
which locates the mounted dataset, prepares/resumes the checkpoint
directory, and invokes opencood/tools/train.py.
"""
import os
import subprocess
import sys
...
def main():
    ...
    run([sys.executable, entry, "--model", "attfuse"])
