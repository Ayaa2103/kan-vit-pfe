"""
Kaggle kernel entry point for the Day-5 V2X-ViT-classic accuracy
evaluation (AP@0.3/0.5/0.7 on the validation split), using OpenCOOD's own
opencood/tools/inference.py unmodified -- same pattern as
kaggle/day5_eval/day5_eval_kernel.py (AttFuse/KAN-ViT), just for this one
model's net_epoch5.pth. Read-only evaluation of an already trained
checkpoint: no training, no config/hyperparameter changes.
"""
import os
import re
import shutil
